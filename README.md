...
```
## year_2024\day_08\sink\solve_2.py
...
from typing import Dict, List, Callable, Tuple, Literal, Set, Generator, Any, DefaultDict
...
# def yield_antinodes_from(a: Tuple[int, int], b: Tuple[int, int], shape: Tuple[int, int], add_origin_to_seen: bool = False):  # List[Tuple[int, int]]
def yield_antinodes_from(a: Tuple[int, int], b: Tuple[int, int], shape: Tuple[int, int]):  # List[Tuple[int, int]]
    height, width = shape
    y_diff_0, x_diff_0 = a[0] - b[0], a[1] - b[1]
    y_diff_1, x_diff_1 = b[0] - a[0], b[1] - a[1]

    seen: Set[Tuple[int, int]] = set()
    # seen: Set[Tuple[int, int]] = {a, b} if add_origin_to_seen else set()
    antinodes: Set[Tuple[int, int]] = set()
    queue = [a, b]

    in_bounds = lambda antinode: -1 < antinode[0] < height and -1 < antinode[1] < width
    add = compose(antinodes.add, queue.append, seen.add)

    while queue:
        pos = queue.pop()
        candidate_0 = pos[0] + y_diff_0, pos[1] + x_diff_0
        candidate_1 = pos[0] + y_diff_1, pos[1] + x_diff_1

        if in_bounds(candidate_0) and candidate_0 not in seen:
            # add(candidate_0)
            antinodes.add(candidate_0), queue.append(candidate_0)

        if in_bounds(candidate_1) and candidate_0 not in seen:
            # add(candidate_1)
            antinodes.add(candidate_1), queue.append(candidate_1)
...
def solve(__input=None):
    ...
    height, width = len(seed), len(seed[0])
    ...
    # m = np.full((height, width), dtype=str, fill_value=".")

    # print(m)
    #  anti_nodes_mat = np.full((height, width), dtype=str, fill_value=".")
    total = 0
    antinodes: Set[Tuple[int, int]] = set()
    for antenna_name, coords in antennas.items():
        for a, b in itertools.combinations(coords, 2):
            for antinode in yield_antinodes_from(a, b, (height, width)):
                antinodes.add(antinode)

    mat = np.full((height, width), dtype=str, fill_value=".")
    for antinode in antinodes:
        mat[antinode] = "#"

    for antenna_name, coords in antennas.items():
        for y, x in coords:
            mat[y, x] = antenna_name
...
```
## year_2024\day_07\sink\solve_2.py
...
Concat = Callable[[int, int], int]

def make_concat(expect: int):
    def inner(left: int, right: int):
        quotient = right
        while left < expect and (quotient := quotient // 10):
            left *= 10
...
def power_of_concat(left: int, right: int):
    quotient = right
...
def str_concat(left: int, right: int):
...
class MathAutomaton:
    ...
    def __init__(self, expect: int, start: int):
        self.expect = expect
        self.state = [start]
        self.concat = power_of_concat  # make_concat(expect) # prefer_fn or concat

    def eval(self, value: int):
        ops = [(operator.mul, "*"), (operator.add, "+"), (self.concat, "||")]
        # ops = [(operator.mul, "*"), (operator.add, "+"), (self.concat, "||")]
    ...
    def is_accepting(self):
        return any(1 for v in self.state if v == self.expect)
...
def solve(__input=None):
    ...
    uniq = {r[0] for r in lines}
    assert len(uniq) == len(lines)

    total = 0
    for row in lines:
        a = MathAutomaton(row[0], row[1])
        for value in row[2:]:
            a.eval(value)
...
```
## year_2024\day_03\sink\solve.py
...
import sys
from collections import Counter, OrderedDict, defaultdict
from typing import Dict, List, Callable, Tuple, Literal, Set, Generator
import numpy as np
from defaultlist import defaultlist
from more_itertools.recipes import sliding_window
from aoc.helpers import locate, build_location, read_lines
...
def solve_1(input_=None):
    ...
    seed_default_list = defaultlist(list)
    with open(locate(input_), "r") as fp:
        for line in read_lines(fp):
            digits = re.findall(r"mul\((\d+),(\d+)\)", line)
            for d in digits:
                seed.append((int(d[0]), int(d[1])))

    # print(ds)

    return sum(i * j for i, j in seed)
...
def solve_2(input_=None):
    ...
    with open(locate(input_), "r") as fp:
        for line in read_lines(fp):
            digits = re.finditer(r"mul\(\d+,\d+\)|don't\(\)|do\(\)", line)

            for ddd in digits:
                seed.append(ddd)

    # seed.sort(key=sortMatch)
    prod = []
    on = True
    for cmd in seed:
        if cmd[0].startswith("do("):
            on = True
        elif cmd[0].startswith("don"):
            on = False
        elif on:
            matches = re.findall(r"mul\((\d+),(\d+)\)", cmd[0])
            for m in matches:
                prod.append((int(m[0]), int(m[1])))

    print(prod)
    # 76413802 - That's not the right answer. If you're stuck, make sure you're using the full input data; there are also some genera
